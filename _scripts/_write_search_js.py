#!/usr/bin/env python3
"""Writes tnet-search-m.js v1.2"""

content = """\
/*
 * tnet-search-m.js  v1.2
...
(function () {
    ...
    /** Layer im MapPlus LayerManager aktivieren */
    function activateLayer(layerId) {
        try {
            var lyrmgr = njs.AppManager.Maps['main'].lyrmgr;
            if (lyrmgr && lyrmgr.addLayer) { lyrmgr.addLayer(layerId); return true; }
        } catch (e) {}
        try {
            if (njs.LayerManager && njs.LayerManager.addLayer) {
                njs.LayerManager.addLayer(layerId); return true;
            }
        } catch (e) {}
        return false;
    }
    ...
    function makeItem(item) {
        ...
        li.innerHTML = icon + '<span class="m-search-item-label">' + (item.label || '') + '</span>';
...
import os
out = r'c:\_Daten\mapplus-exp\maps\tnet\js\mobile\tnet-search-m.js'
