...
(function () {
  ...
  function activateLegacyPrintFrameWithRetry() {
    ...
    function tryActivate() {
      ...
      if (tpWidget || printWidget || tpDetails) {
        ...
        try {
          if (window.njs_pdfscale_list && typeof window.njs_pdfscale_list.get === 'function' && typeof window.njs_pdfscale_list.set === 'function') {
            window.njs_pdfscale_list.set('value', window.njs_pdfscale_list.get('value'));
          } else {
            var legacyScaleEl = document.getElementById('njs_pdfscale_list');
            if (legacyScaleEl) legacyScaleEl.dispatchEvent(new Event('change', { bubbles: true }));
          }
        } catch (refreshErr) {
      ...
      if (attempt < maxAttempts) setTimeout(tryActivate, 150);
      else TnetLog.warn('[Drucken] Legacy-Print Widgets nicht gefunden (provider=mapplus)');
    }
