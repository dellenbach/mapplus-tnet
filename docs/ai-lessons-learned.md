...
- **Fix**: `RAW_CONF_DIR` in der API und die AGS-Statusmeldungen im UI auf `/data/Client_Data/nwow/raw-conf` vereinheitlicht; widersprüchliche Lessons-Learned-Stelle angepasst.
...
- **Fix**: `RAW_CONF_DIR` auf `/data/Client_Data/nwow/raw-conf` vereinheitlicht.
